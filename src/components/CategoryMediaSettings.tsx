...
export function CategoryMediaSettings({ categories, enabled }: { categories: Category[]; enabled: boolean }) {
  ...
  const persistedPhotos = category ? (saved[category.id] ?? category.capturePhotos ?? false) : false;
  const unchanged = Boolean(category) && photos === persistedPhotos;
  useEffect(() => {
  ...
  async function save() {
    ...
    try {
      ...
      if (data.capturePhotos !== photos) throw new Error('Сервер не подтвердил настройку фотографий');
  ...
  return <section className="rounded-xl border border-zinc-700 bg-zinc-900 p-5 space-y-4">
    ...
    <div className="relative">
      <select aria-label="Категория для фотографий" disabled={busy} value={selected} onChange={event => { setSelected(event.target.value); setNotice(''); }} className="w-full appearance-none rounded-lg border border-zinc-700 bg-zinc-950 py-3 pl-3 pr-12 text-white disabled:opacity-50">
    ...
    <button type="button" onClick={save} disabled={!category || busy || !enabled || unchanged} className="rounded bg-accent px-4 py-3 font-bold text-black disabled:cursor-not-allowed disabled:opacity-40">{busy ? 'Сохранение…' : unchanged ? 'Сохранено' : 'Сохранить фотографии'}</button>
